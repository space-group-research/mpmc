...
double
disp_expansion_lrc(const system_t *system, pair_t *pair_ptr, const double cutoff) /* ignoring the exponential repulsion bit because it decays exponentially */
{
    if (!(pair_ptr->frozen) &&  /* disqualify frozen pairs */
            ((pair_ptr->lrc == 0.0) || pair_ptr->last_volume != system->pbc->volume))
...
double disp_expansion_lrc_self(const system_t *system, atom_t *atom_ptr, const double cutoff)
{
    if (!(atom_ptr->frozen) && /* disqualify frozen atoms */
            ((atom_ptr->lrc_self == 0.0) || atom_ptr->last_volume != system->pbc->volume))
    { /* LRC only changes if the volume change) */
