...
void
pair_exclusions(system_t *system, molecule_t *molecule_i, molecule_t *molecule_j, atom_t *atom_i, atom_t *atom_j, pair_t *pair_ptr)
{
    ...
    if ((molecule_i == molecule_j) && !system->gwp)
    { /* if both on same molecule, exclude all interactions */
    ...
    if (!system->sg)
    {
        if (system->waldmanhagler && !system->cdvdw_sig_repulsion)
        { //wh mixing rule
            si3 = atom_i->sigma;
            si3 *= si3 * si3;
            si6 = si3 * si3;
        ...
        } else if (system->halgren_mixing)
        { //halgren mixing rules
            if (atom_i->sigma > 0.0 && atom_j->sigma > 0.0)
            {
        ...
        } else if (system->cdvdw_9th_repulsion)
        { //9th power mixing for repulsion
            si3 = atom_i->sigma;
            si3 *= si3 * si3;
            si6 = si3 * si3;
            sj3 = atom_j->sigma;
        ...
        } else if (system->cdvdw_sig_repulsion)
        { //sigma repulsion for coupled-dipole vdw
            si3 = atom_i->sigma;
            si3 *= si3 * si3;
            si6 = si3 * si3;
            sj3 = atom_j->sigma;
        ...
        {// mix for buckingham repulsion
            ...
            } else if (system->bohm_ahlrichs_mixing)
            {
                ...
                pair_ptr->sigma = log(Aij / c) / Bij;
            } else if (system->wilson_popelier_mixing)
            {
                double c = 315.7750382111558307123944638;
                double Aii = c * exp(atom_i->epsilon * atom_i->sigma);
                double Ajj = c * exp(atom_j->epsilon * atom_j->sigma);
    ...
    if (system->spectre)
    {
        ...
        if (atom_i->spectre && atom_j->spectre)
        { /* case for both spectre */

            pair_ptr->rd_excluded = 0;
            pair_ptr->es_excluded = 1;

        } else if (atom_i->spectre || atom_j->spectre)
        { /* case for one spectre */
