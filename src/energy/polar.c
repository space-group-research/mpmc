    ...
    } else if (system->polar_iterative)
    {
        ...
        if (system->iter_success)
        {
            switch (system->ensemble)
            {
                ...
                case ENSEMBLE_NVT:
                case ENSEMBLE_NVE:
                case ENSEMBLE_NPT: sprintf(linebuf, "POLAR: polarization iterative solver convergence failure on mc step %d.\n", system->step);
                    error(linebuf);
                    break;
                case ENSEMBLE_REPLAY: sprintf(linebuf, "POLAR: polarization iterative solver convergence failure on configuration %d.\n", system->step);
                    error(linebuf);
                    break;
                case ENSEMBLE_SURF:
                case ENSEMBLE_SURF_FIT:
                case ENSEMBLE_TE:
                default: sprintf(linebuf, "POLAR: polarization iterative solver failed to reach convergence.\n");
                    error(linebuf);
            }
        }
