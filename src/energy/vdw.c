...
double calc_e_iso(system_t *system, double *sqrtKinv, molecule_t *mptr)
{
    ...
    {
        if (molecule_ptr != mptr)
        {  //count atoms then skip to next molecule
...
double sum_eiso_vdw(system_t *system, double *sqrtKinv)
{
    ...
    {
        for (vp = system->vdw_eiso_info; vp != NULL; vp = vp->next)
        { //loop through all vp's
        ...
        if (vp == NULL)
        { //if the molecule was unmatched, we need to grow the list
            ...
            if (isfinite(vpscan->energy) == 0)
            { //if nan, then calc_e_iso failed
        ...
        for (atom_ptr = molecule_ptr->atoms; atom_ptr; atom_ptr = atom_ptr->next)
        {
            for (pair_ptr = atom_ptr->pairs; pair_ptr; pair_ptr = pair_ptr->next)
            {
                ...
                if (atom_ptr->polarizability == 0 || pair_ptr->atom->polarizability == 0 ||
                        atom_ptr->omega == 0 || pair_ptr->atom->omega == 0)
                    continue; //no vdw energy
