...
void population_histogram(system_t *system) {
    ...
    for (mol_p = system->molecules; mol_p; mol_p = mol_p->next) {
        if (!mol_p->frozen) {
            ...
            wrap1coord(mol_p->com, wrappedcoords, system);
...
void write_histogram(FILE *fp_out, int ***grid, system_t *system) {
    ...
    rewind(fp_out);
    fprintf(fp_out,
            "# OpenDX format population histogram\n");
    fprintf(fp_out,
    ...
    for (i = 0; i < xdim; i++) {
        for (j = 0; j < ydim; j++) {
            for (k = 0; k < zdim; k++) {
                fprintf(fp_out,
                        "%f ", (float)(grid[i][j][k]) / (float)(system->grids->avg_histogram->norm_total));
                count += grid[i][j][k];
            }
            fprintf(fp_out,
                    "\n");
...
void setup_histogram(system_t *system) {
    ...
    sprintf(linebuf,
            "HISTOGRAM: %f resolution -> %d bins(A) * %d bins(B) * %d bins(C)\n", system->hist_resolution, x_dim, y_dim, z_dim);
    output(linebuf);

    Nbins = x_dim * y_dim * z_dim;
    sprintf(linebuf,
            "HISTOGRAM: Total Bins = %d\n", Nbins);
    output(linebuf);
