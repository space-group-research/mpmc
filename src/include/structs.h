...
typedef struct _pair {
  ...
  double last_volume; // what was the volume when we last calculated LRC? needed
                      // for NPT
...
typedef struct _atom {
  ...
  double pos[3],
      wrapped_pos[3]; // absolute and wrapped (into main unit cell) position
  ...
  int gwp_spin;
  double gwp_alpha;
  int site_neighbor_id; // dr fluctuations will be applied along the vector from
                        // this atom to the atom identified by this variable
  pair_t *pairs;
...
typedef struct _molecule {
  ...
  double quantum_rotational_potential_grid[QUANTUM_ROTATION_GRID]
                                          [QUANTUM_ROTATION_GRID];
#endif /* QM_ROTATION */
#ifdef XXX
...
typedef struct _sorbateAverages {
  ...
  double percent_wt, percent_wt_sq,
      percent_wt_err; // weight percent for this sorbate (sorb_mass /
                      // total_mass)
  double percent_wt_me, percent_wt_me_sq,
      percent_wt_me_err; // weight percent for this sorbate (sorb_mass /
                         // frozen_mass)
  double excess_ratio, excess_ratio_sq,
      excess_ratio_err; // excess adsorption ratio
  double pore_density, pore_density_sq,
      pore_density_err;                    // mass / volume of pore
  double density, density_sq, density_err; // mass of sorbate / volume
  double selectivity,
      selectivity_err; // sorbate's selectivity ratio relative to all other
                       // sorbates in the insert list.
...
typedef struct _message {
        ...
        double acceptance_rate;
        double acceptance_rate_insert, acceptance_rate_remove,
acceptance_rate_displace; double acceptance_rate_spinflip,
acceptance_rate_volume; double cavity_bias_probability,
cavity_bias_probability_sq; double polarization_iterations,
polarization_iterations_sq; int ***grid; } message_t;
*/
...
typedef struct _fileNode {
  intechar data; // data.count is for use in the list head only & indicates the
                 // # of list elements data.filename is for use in the nodes
...
typedef struct _system {
  ...
  // auto-reject options
  // first is in terms of sigma and only applies to LJ; latter is in Angstroms
  // and applies to all pairs
  ...
  int iter_success; // flag set when iterative solver fails to converge (when
                    // polar_precision is used)
  ...
  int surf_print_level; // sets the amount of output (1-6) that correspond to
                        // the nested loops in surface.c
  ...
  int num_insertion_molecules; // the number of elements found in both lists
                               // below:
  molecule_t
      *insertion_molecules; // linked list of molecules to be randomly inserted
  molecule_t **insertion_molecules_array; // array providing direct access to
                                          // elements of above list
