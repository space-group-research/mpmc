...
void check_ensemble(system_t *system, int ensemble)
{
    ...
    switch (ensemble)
    {
        case ENSEMBLE_UVT: output("INPUT: Grand canonical ensemble\n");
            break;
        case ENSEMBLE_NVT: output("INPUT: Canonical ensemble\n");
            break;
        case ENSEMBLE_SURF: output("INPUT: Potential energy surface\n");
            break;
        case ENSEMBLE_SURF_FIT: output("INPUT: Potential energy surface fitting\n");
            break;
        case ENSEMBLE_NVE: output("INPUT: Microcanonical ensemble\n");
            break;
        case ENSEMBLE_TE: output("INPUT: Single-point energy calculation\n");
            system->numsteps = 0;
            system->corrtime = 0;
            break;
        case ENSEMBLE_NPT: output("INPUT: Isobaric-Isothermal ensemble\n");
            break;
        case ENSEMBLE_REPLAY: output("INPUT: Replaying trajectory\n");
            break;
        default: error("INPUT: improper ensemble specified\n");
            die(-1);
    }
...
void feynman_hibbs_options(system_t *system)
{
    ...
    } else
    {
        ...
        {
            case 2: sprintf(linebuf, "INPUT: Feynman-Hibbs second-order quantum correction activated\n");
                output(linebuf);
                break;
            case 4: sprintf(linebuf, "INPUT: Feynman-Hibbs fourth-order quantum correction activated\n");
                output(linebuf);
                break;
            default: output("INPUT: Feynman-Hibbs order unspecified - defaulting to h^2\n");
...
void io_files_options(system_t *system)
{
...
#endif
        output(linebuf);
    } else if (!strcasecmp(system->pqr_restart, "off"))
    {    // Optionally turn off restart configuration output
    ...
    {    // (CRC)
        ...
        output(linebuf);
#endif
    } else if (!strcasecmp(system->pqr_output, "off"))
    {    // Optionally turn off final configuration output
    ...
    {    // (CRC)
        ...
        output(linebuf);
    } else if (!strcasecmp(system->energy_output, "off"))
    {    // Optionally turn off energy printing
    ...
    { //if we are doing virial
        if (!system->virial_output)
        {
            ...
            output(linebuf);
        } else if (!strcasecmp(system->virial_output, "off"))
        { //optionally turn off virial printing
    ...
    {    // (CRC)
        ...
        output(linebuf);
    } else if (!strcasecmp(system->traj_output, "off"))
    {    // Optionally turn off trajectory printing
...
int check_system(system_t *system)
{
    ...
    if (system->schmidt_mixing) output("INPUT: Using the Schmidt mixing rule for exponential repulsions with disp_expansion.\n");
    if (system->gilbert_smith_mixing) output("INPUT: Using the Gilbert Smith mixing rule for exponential repulsions with disp_expansion.\n");
    if (system->bohm_ahlrichs_mixing) output("INPUT: Using the Bohm Ahlrichs mixing rule for exponential repulsions with disp_expansion.\n");
    if (system->wilson_popelier_mixing) output("INPUT: Using the Wilson Popelier mixing rule for exponential repulsions with disp_expansion.\n");
