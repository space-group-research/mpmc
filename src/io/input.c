...
int do_command(system_t *system, char **token)
{
    ...
    } else if (!strcasecmp(token[0], "user_fugacities"))
    { //read in a list of user-defined fugacities
...
curveData_t *readFitInputFiles(system_t *system, int nCurves)
{
    ...
    while (fNode)
    {
        ...
        while (fgets(linebuf, MAXLINE, fp_fit))
        {
            ...
            else if ((token1[0] == '*') || (token1[0] == 0)); // Do nothing for comment lines and blank lines
            else
            {
