...
void terminate_handler(int sigtype, system_t *sys_ptr)
{
    ...
    switch (sigtype)
    {
        ...
        case SIGTERM: output("CLEANUP: ************ SIGTERM received, exiting *************\n");
            close_files(sys_ptr);
            cleanup(system);
#ifdef MPI
        ...
        case SIGUSR1: output("CLEANUP: ************ SIGUSR1 received, exiting *************\n");
            close_files(sys_ptr);
            cleanup(system);
#ifdef MPI
            if(!rank) close_files(sys_ptr);
        ...
        case SIGUSR2: output("CLEANUP: ************ SIGUSR2 received, exiting *************\n");
            close_files(sys_ptr);
            cleanup(system);
#ifdef MPI
            if(!rank) close_files(sys_ptr);
