...
int main(int argc, char **argv) {
...
#ifndef __WIN32__
        sprintf(linebuf,
                "MPMC (Massively Parallel Monte Carlo) r%d - 2012-2019 GNU Public "
                "License\n",
                VERSION);
        output(linebuf);
#endif
    ...
    if (access("/proc/cpuinfo", R_OK) != -1) { // Linux
        ...
        sprintf(linebuf, "MAIN: Job running on node -> %.400s", nodename);
        output(linebuf);
        pclose(host);
        ...
        while (!feof(procfile)) {
            ...
            if (strncasecmp(cpu, "model name", 10) == 0) {
                sprintf(linebuf, "MAIN: CPU -> %.400s", cpu);
                output(linebuf);
                break;
            }
        }
    ...
    sprintf(linebuf, "MAIN: running parameters found in %.400s\n", input_file);
    output(linebuf);
    ...
    memnullcheck(system->avg_nodestats, sizeof(avg_nodestats_t), __LINE__ - 1,
            __FILE__);
    system->observables = calloc(1, sizeof(observables_t));
    memnullcheck(system->observables, sizeof(observables_t), __LINE__ - 1,
            __FILE__);
    system->avg_observables = calloc(1, sizeof(avg_observables_t));
    memnullcheck(system->avg_observables, sizeof(avg_observables_t), __LINE__ - 1,
            __FILE__);
    system->checkpoint = calloc(1, sizeof(checkpoint_t));
    memnullcheck(system->checkpoint, sizeof(checkpoint_t), __LINE__ - 1,
            __FILE__);
    system->checkpoint->observables = calloc(1, sizeof(observables_t));
    memnullcheck(system->checkpoint->observables, sizeof(observables_t),
            __LINE__ - 1, __FILE__);
    ...
    memnullcheck(system->grids->histogram, sizeof(histogram_t), __LINE__ - 1,
            __FILE__);
    system->grids->avg_histogram = calloc(1, sizeof(histogram_t));
    memnullcheck(system->grids->avg_histogram, sizeof(histogram_t), __LINE__ - 1,
            __FILE__);
    ...
    if (system->ensemble == ENSEMBLE_SURF) { /* surface */
        ...
        if (system->surf_fit_multi_configs) {
            if (surface_multi_fit(system) < 0) {
                error("MAIN: surface fitting module (for multiple configurations) "
                        "failed on error, exiting\n");
                die(1);
            }
        } else if (system->surf_fit_arbitrary_configs) {
            if (surface_fit_arbitrary(system) < 0) {
                error("MAIN: surface fitting module (for arbitrary configurations) "
                        "failed on error, exiting\n");
                die(1);
            }
    ...
    else if (system->ensemble ==
            ENSEMBLE_REPLAY) { /* replay trajectory and recalc energies, etc. */
