...
void boltzmann_factor(system_t *system, double initial_energy, double final_energy, double rot_partfunc)
{
    ...
    switch (system->ensemble)
    {
        ...
        case ENSEMBLE_UVT :
            ...
            if (system->cavity_bias && system->checkpoint->biased_move)
            {
                ...
                switch (system->checkpoint->movetype)
                {
                    ...
                    case MOVETYPE_REMOVE :
                        ...
                        break;
                    case MOVETYPE_DISPLACE : system->nodestats->boltzmann_factor = exp(-delta_energy / system->temperature);
                        break;
                    default : error("MC: invalid mc move (not implemented for biased moves?)\n");
                        die(-1);
                } //mc move switch
            } //end biased move
            else
            {
                switch (system->checkpoint->movetype)
                {
                    ...
                    case MOVETYPE_DISPLACE : system->nodestats->boltzmann_factor = exp(-delta_energy / system->temperature);
                        break;
                    case MOVETYPE_SPINFLIP : system->nodestats->boltzmann_factor = rot_partfunc * exp(-delta_energy / system->temperature);
                        break;
                    default: error("MC: invalid mc move (not implemented for binary mixtures?)\n");
                        die(-1);
                }//MC move switch
            } //end biased or not?
            break; //end UVT
        ...
        case ENSEMBLE_NVE : system->nodestats->boltzmann_factor = pow((system->total_energy - final_energy), 3.0 * system->N / 2.0);
            system->nodestats->boltzmann_factor /= pow((system->total_energy - initial_energy), 3.0 * system->N / 2.0);
            break;

        default: error("MC: invalid ensemble. aborting.\n");
            die(-1);
    }
...
void register_accept(system_t *system)
{
    ...
    switch (system->checkpoint->movetype)
    {
        ...
        case MOVETYPE_INSERT: ++system->nodestats->accept_insert;
            break;
        case MOVETYPE_REMOVE: ++system->nodestats->accept_remove;
            break;
        case MOVETYPE_DISPLACE: ++system->nodestats->accept_displace;
            break;
        case MOVETYPE_ADIABATIC: ++system->nodestats->accept_adiabatic;
            break;
        case MOVETYPE_SPINFLIP: ++system->nodestats->accept_spinflip;
            break;
        case MOVETYPE_VOLUME: ++system->nodestats->accept_volume;
            break;
    }
...
void register_reject(system_t *system)
{
    ...
    switch (system->checkpoint->movetype)
    {
        ...
        case MOVETYPE_INSERT: ++system->nodestats->reject_insert;
            break;
        case MOVETYPE_REMOVE: ++system->nodestats->reject_remove;
            break;
        case MOVETYPE_DISPLACE: ++system->nodestats->reject_displace;
            break;
        case MOVETYPE_ADIABATIC: ++system->nodestats->reject_adiabatic;
            break;
        case MOVETYPE_SPINFLIP: ++system->nodestats->reject_spinflip;
            break;
        case MOVETYPE_VOLUME: ++system->nodestats->reject_volume;
            break;
