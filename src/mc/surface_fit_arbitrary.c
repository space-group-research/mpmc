...
double
error_calc_fr_arbitrary_configurations(system_t *system, int nConfigs, configData_t *configuration, double max_energy)
{
...
void
get_configuration_energies(system_t *system, configData_t *configurations, int nConfigs, param_g *params, configData_t *orig)
{
...
//	configData_t *config = (*c);  // for slightly more convenient referencing
    ...
    while (fNode)
    {
        ...
        while (fgets(linebuf, MAXLINE, fp_fit))  // read config record header
        {
            ...
            if (fgets(linebuf, MAXLINE, fp_fit))
            {  // read the energy
