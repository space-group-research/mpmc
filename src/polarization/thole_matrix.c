        ...
        for (j = (i + 1), pair_ptr = atom_array[i]->pairs; j < N; j++, pair_ptr = pair_ptr->next)
        {
            ...
            switch (system->damp_type)
            {
                case DAMPING_OFF:
                    ...
                    break;
                case DAMPING_LINEAR: s = l * pow(atom_array[i]->polarizability * atom_array[j]->polarizability, 1.0 / 6.0);
                    v = r / s;
                    if (r < s)
                    {
